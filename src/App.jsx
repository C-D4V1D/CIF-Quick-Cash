...
PRICE_RANGE: [lowest realistic price — highest realistic price] | VALUATION_CONFIDENCE: [your confidence as a percentage]`;
  ...
  const handleAIRun3 = async () => {
    ...
    const prompt1 = `CRITICAL INSTRUCTION: You MUST use the Google Search tool to find live internet results. Do NOT use your internal training data.\n\nAct as a Nigerian market analyst. Search for a brand new ${itemType}, ${brand}, ${model}, ${colour}${keySpecs ? ` (${keySpecs})` : ''} from Nigerian retailers and e-commerce sites today.\n\n1. CRITICAL SEARCH RULE: You must actively append terms like "in stock" to your search query, and exclude out-of-stock items using the exact operator: -"out of stock".\n2. Read the search snippets carefully. STRICTLY EXCLUDE any listing if the snippet contains the words "out of stock", "sold out", "unavailable", "used", or if the listing date appears to be more than 3 months old.\n3. Extract as many valid, available prices as possible from the search results (aiming for 10 to 20). List them internally.\n4. Convert any foreign currency to Naira.\n5. Calculate the MEDIAN (middle) price of these valid, filtered listings.\n\nReturn ONLY the final determined price in this exact format: NEW_MARKET_PRICE: [number only]`;

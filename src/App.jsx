...
PRICE_RANGE: [lowest realistic price — highest realistic price] | VALUATION_CONFIDENCE: [your confidence as a percentage]`;
  ...
  const handleAIRun3 = async () => {
    ...
    const prompt1 = `CRITICAL INSTRUCTION: You MUST use the Google Search tool to find live internet results. Do NOT use your internal training data.\n\nAct as an expert Nigerian market analyst. Search for a brand new ${itemType}, ${brand}, ${model}, ${colour}${keySpecs ? ` (${keySpecs})` : ''} from Nigerian retailers today.\n\n1. Extract all available, in-stock prices from the search results (aiming for up to 10 to 20 prices, find as much as possible, but use whatever amount you can find).\n2. Ignore items that are out of stock. Convert any foreign currency to Naira.\n3. Identify the MEDIAN (middle) price among the extracted list.\n\nReturn ONLY the final determined MEDIAN price in this exact format: NEW_MARKET_PRICE: [number only]`;

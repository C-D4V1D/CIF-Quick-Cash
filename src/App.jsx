...
function ScreeningStep({ tx, upd, onRedFlagExit, onDecline }) {
  ...
  return (
    <div>
      ...
      <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>How long have you had this item?<InfoIcon tip="Short ownership period (under 6 months) combined with other red flags may indicate a recently stolen item. Note any inconsistencies." /></span>} required>
      ...
      <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Where did you buy it?<InfoIcon tip="Open market and online purchases can be harder to verify. If the customer is vague or the answer changes, treat this as a potential red flag." /></span>} required>
      ...
      <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Is this item registered in your name?<InfoIcon tip="For phones, the SIM and IMEI ideally match the customer's identity. 'N/A' is appropriate for items like fans and speakers that don't require registration." /></span>} required>
      ...
      <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Has anyone (or is anyone) else used/using this item with you?<InfoIcon tip="If the item is shared or belongs to someone else (employer, spouse, etc.), make sure the customer has authority to pledge it as collateral." /></span>} required>
      ...
      <div style={{ marginTop: '16px', padding: '16px', background: COLORS.dangerLight, borderRadius: '8px', border: '1px solid #f5c6cb' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
          ...
          <InfoIcon tip="Tick this if you have a gut feeling something is wrong — inconsistent story, nervous behaviour, item with suspicious markings, mismatched answers. The system will decline the transaction discreetly without showing any reason to the customer." />
        </label>
        {tx.screeningRedFlag && (
...
function CaptureStep({ tx, upd, settings, onJumpToOffer, onEndTransaction, onDecline }) {
  ...
  return (
    <div>
      ...
      {/* 6A — Item Type */}
      <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Item Type<InfoIcon tip="Select the category that best matches what the customer has brought in. This determines which photo slots, inspection checklist, and IMEI requirements apply." /></span>} required>
        <select
          style={S.select}
      ...
      {showCaptureBody && (
        <>
          ...
          {/* 6D — Receipt */}
          <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: `2px solid ${COLORS.border}` }}>
            <div style={{ fontSize: '14px', fontWeight: 700, marginBottom: '8px', display: 'flex', alignItems: 'center' }}>🧾 Original Purchase Receipt<InfoIcon tip="A receipt proves legal ownership and lets us offer a higher advance (up to the 'with receipt' cap in Settings). Without a receipt, we use the lower cap to limit risk." /></div>
    ...
    <div>
      ...
      <div style={{ marginTop: '4px' }}>
        <div style={{ fontSize: '14px', fontWeight: 700, marginBottom: '6px', display: 'flex', alignItems: 'center' }}>
          ...
          <InfoIcon tip="This is an internal-only record. Use it to document anything you noticed during inspection: unusual smells, loose parts, battery concerns, missing accessories, or why you skipped a checklist item." />
        </div>
  ...
  const TxDetail = ({ tx }) => {
    ...
    return (<div>
      ...
      <div style={S.card}>
        ...
        <div style={S.grid4}>
          <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Estimated Value<InfoIcon tip="Market resale value estimated by AI from photos. The maximum loan advance is a percentage of this figure." position="bottom" /></div><div style={S.statValue}>{fmtMoney(tx.estimatedValue)}</div></div>
          <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Cash Advanced<InfoIcon tip="The actual amount of money given to the customer when they brought in the item." position="bottom" /></div><div style={S.statValue}>{fmtMoney(tx.cashAdvance)}</div></div>
          {tx.type === 'advance' && <>
            <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Days Outstanding<InfoIcon tip="Number of days since the cash advance was given. Daily interest fees accumulate for each of these days." position="bottom" /></div><div style={S.statValue}>{daysOut}d</div></div>
            <div style={{ ...S.stat, background: tx.status === 'active' ? COLORS.dangerLight : COLORS.primaryLight }}>
              <div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Amount Due Today<InfoIcon tip="Total the customer owes right now: cash advanced + (daily interest × days outstanding). This grows every day." position="bottom" /></div>
      ...
      {tx.type === 'advance' && timeline && (
        <div style={S.card}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px', marginBottom: '12px' }}>
            {[
              { label: 'Date Given', date: tx.dateGiven, bg: COLORS.bg, fg: COLORS.text, border: COLORS.border, tip: 'The date the cash advance was handed to the customer and the loan began.' },
              { label: 'Agreed Return', date: tx.deadlineDate, bg: customerDaysLeft !== null && customerDaysLeft <= 0 ? COLORS.dangerLight : COLORS.bg, fg: customerDaysLeft !== null && customerDaysLeft <= 0 ? COLORS.danger : COLORS.text, border: customerDaysLeft !== null && customerDaysLeft <= 0 ? '#f5c6cb' : COLORS.border, tip: 'The date the customer promised to return and pay back their loan. Contact them before this date.' },
              { label: 'Internal Deadline', date: timeline.internal_deadline, bg: COLORS.bg, fg: COLORS.text, border: COLORS.border, tip: 'An internal staff deadline set before the customer\'s agreed return. Begin contact attempts by this date to avoid the grace period.' },
              { label: 'Grace Period Ends', date: timeline.grace_end_date, bg: '#f3e8ff', fg: '#7c3aed', border: '#d8b4fe', tip: 'The last day of the buffer window after the internal deadline. Once this passes, the item becomes eligible for sale.' },
              { label: 'Sale Eligible From', date: timeline.sale_allowed_date, bg: '#f0fdf4', fg: '#166534', border: '#86efac', tip: 'From this date, if the customer has not repaid, the item can be sold to recover the cash advance.' },
  ...
  const renderPage = () => {
    ...
    switch (page) {
      case 'dashboard': return (<div>{listLoadingNotice}
        ...
        <div style={S.grid4}>
          <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Available Lending Capital<InfoIcon tip="Cash you can lend out right now. Calculated as: total invested capital + all-time profit − money out on active loans − for-sale inventory value − profit already distributed." /></div><div style={S.statValue}>{fmtMoney(availableLendingCapital)}</div></div>
          <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Capital Out<InfoIcon tip="Total cash currently loaned out across all active (unreturned) transactions." /></div><div style={S.statValue}>{fmtMoney(totalCapitalOut)}</div></div>
          <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Active Loans<InfoIcon tip="Number of transactions where customers have not yet returned to collect their items." /></div><div style={S.statValue}>{activeTxs.length}</div></div>
          <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Revenue<InfoIcon tip="All-time money received from repayment fees, item sales, and service charges combined." /></div><div style={S.statValue}>{fmtMoney(totalRevenue)}</div></div>
          <div style={{ ...S.stat, background: inGracePeriod.length > 0 ? '#f3e8ff' : COLORS.primaryLight }}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>In Grace Period<InfoIcon tip="Loans past the customer's agreed return date, but still within the internal buffer window before items are listed for sale." /></div><div style={{ ...S.statValue, color: inGracePeriod.length > 0 ? '#7c3aed' : COLORS.primary }}>{inGracePeriod.length}</div></div>
          <div style={{ ...S.stat, background: readyToSell.length > 0 ? COLORS.dangerLight : COLORS.primaryLight }}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Ready to Sell<InfoIcon tip="Items that have passed the grace period and can now be sold to recover the cash advance." /></div><div style={{ ...S.statValue, color: readyToSell.length > 0 ? COLORS.danger : COLORS.primary }}>{readyToSell.length}</div></div>
        </div>
      ...
      case 'deadlines': {
        ...
        return (<div>{listLoadingNotice}<h2 style={{ fontSize: '20px', fontWeight: 800, marginBottom: '20px', color: COLORS.primaryDark }}>🔔 Deadlines & Alerts</h2><AlertGroup title="READY TO SELL" items={readyToSell} color="#1e1e1e" icon="🏷" infoTip="Items that have passed the internal grace period deadline. They can now be listed and sold to recover the cash advance." /><AlertGroup title="GRACE PERIOD" items={inGrace} color="#7c3aed" icon="⏰" infoTip="Loans past the customer's agreed return date but still within the internal grace buffer. Contact the customer urgently — the item will become sellable once grace ends." /><AlertGroup title="7 DAYS OR LESS" items={upcoming7} color="#f59e0b" icon="📅" infoTip="Customers who have 7 days or fewer remaining before their agreed return date. Start contact attempts now." />{readyToSell.length + inGrace.length + upcoming7.length === 0 && <div style={S.card}><p style={{ color: COLORS.textMuted, textAlign: 'center' }}>All clear! ✅</p></div>}</div>);
      }
        ...
        return (
          <div>
            ...
            <div style={S.card}>
              <div style={S.cardTitle}>💰 Financial Summary</div>
              <div style={S.grid3}>
                <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Revenue<InfoIcon tip="All money received this period — from repayment fees (daily interest collected), item sales proceeds, and service charges." position="bottom" /></div><div style={S.statValue}>{fmtMoney(rRevenue)}</div></div>
                <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Expenses<InfoIcon tip="Money spent running the business this period — stationery, printing, transport, airtime, and other operating costs." position="bottom" /></div><div style={{ ...S.statValue, color: COLORS.danger }}>{fmtMoney(rExpTotal)}</div></div>
                <div style={S.stat}><div style={{ ...S.statLabel, display: 'flex', alignItems: 'center' }}>Net Profit<InfoIcon tip="Revenue minus Expenses. This is what the business actually earned after paying all costs. Fabian and stakeholders share this amount." position="bottom" /></div><div style={{ ...S.statValue, color: rProfit >= 0 ? COLORS.primary : COLORS.danger }}>{fmtMoney(rProfit)}</div></div>
              </div>
              <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: `1px solid ${COLORS.border}` }}>
                ...
                <div style={rowStyle}><span>Repayment fees ({rClosed.length} loan{rClosed.length !== 1 ? 's' : ''})<InfoIcon tip="Daily holding charges collected when customers repay their loans and collect their items." /></span><strong style={{ color: COLORS.primary }}>{fmtMoney(rRepaymentFees)}</strong></div>
                <div style={rowStyle}><span>Sales proceeds ({rSold.length} item{rSold.length !== 1 ? 's' : ''})<InfoIcon tip="Money received when items are sold — for customers who did not come back to redeem before the deadline." /></span><strong style={{ color: COLORS.primary }}>{fmtMoney(rSalesRevenue)}</strong></div>
                <div style={{ ...rowStyle, borderBottom: 'none' }}><span>Service fees ({rNewTxs.length} new loan{rNewTxs.length !== 1 ? 's' : ''} × {fmtMoney(settings.serviceFee || 1000)})<InfoIcon tip="One-time charge collected at the start of each new loan, before daily fees begin." /></span><strong style={{ color: COLORS.primary }}>{fmtMoney(rServiceFees)}</strong></div>
              </div>
              <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: `1px solid ${COLORS.border}` }}>
                ...
                <div style={rowStyle}><span>Capital deployed (new loans)<InfoIcon tip="Total loan money given out as cash advances in this period. This money is out in the field and will be returned when customers repay." /></span><strong style={{ color: COLORS.danger }}>{fmtMoney(rCapitalDeployed)}</strong></div>
                <div style={{ ...rowStyle, borderBottom: 'none' }}><span>Capital returned (repayments)<InfoIcon tip="Total loan principal recovered from customers who repaid their loans this period." /></span><strong style={{ color: COLORS.primary }}>{fmtMoney(rCapitalReturned)}</strong></div>
              </div>
            </div>

            {/* Profit distribution */}
            <div style={S.grid2}>
              <div style={S.card}><div style={{ ...S.cardTitle, display: 'flex', alignItems: 'center' }}>Fabian (10%)<InfoIcon tip="Management fee: 10% of the net profit goes to Fabian for running and overseeing the business operations." /></div><div style={{ fontSize: '24px', fontWeight: 800, color: COLORS.accent }}>{fmtMoney(rFabian)}</div></div>
              <div style={S.card}><div style={{ ...S.cardTitle, display: 'flex', alignItems: 'center' }}>Stakeholders<InfoIcon tip="90% of net profit shared among all investors, proportional to each person's capital contribution." /></div><div style={{ fontSize: '24px', fontWeight: 800, color: COLORS.primary }}>{fmtMoney(rStakeholder)}</div></div>
            </div>
            <div style={S.card}>
              <div style={{ ...S.cardTitle, display: 'flex', alignItems: 'center' }}>📊 Stakeholder Distribution<InfoIcon tip="Each stakeholder's share of the 90% profit pool. Calculated from their capital as a percentage of the total invested capital." /></div>
              {rStakeholders.map(s => (
        ...
        return (
          <div>
            ...
            <div style={{ ...S.card, borderLeft: `4px solid ${availableLendingCapital >= 0 ? COLORS.primary : COLORS.danger}`, marginBottom: '16px' }}>
              <div style={{ ...S.cardTitle, display: 'flex', alignItems: 'center' }}>💰 Available for Lending<InfoIcon tip="Cash available to give out as new loans right now. Calculated as: total invested capital + all-time profit − money on active loans − for-sale inventory − distributed profits." /></div>
              ...
              <div style={capRowStyle}><span>Total capital invested<InfoIcon tip="The sum of all money that stakeholders have put into the business." /></span><strong>+ {fmtMoney(totalCapital)}</strong></div>
              <div style={capRowStyle}><span>All-time profit (revenue − expenses)<InfoIcon tip="Total profit the business has earned since inception. This adds to the lending pool." /></span><strong style={{ color: netProfit >= 0 ? COLORS.primary : COLORS.danger }}>+ {fmtMoney(netProfit)}</strong></div>
              <div style={capRowStyle}><span>Money out on active loans<InfoIcon tip="Cash currently loaned out to customers on active transactions. This is temporarily unavailable for new loans." /></span><strong style={{ color: COLORS.danger }}>− {fmtMoney(totalCapitalOut)}</strong></div>
              <div style={capRowStyle}><span>Capital in for-sale inventory<InfoIcon tip="Cash tied up in items that are listed for sale (loans past deadline). Will be recovered when items are sold." /></span><strong style={{ color: COLORS.danger }}>− {fmtMoney(totalCapitalInForSaleInventory)}</strong></div>
              <div style={{ ...capRowStyle, borderBottom: 'none' }}><span>Profit already distributed to stakeholders<InfoIcon tip="Profit that has already been paid out and is no longer available in the business." /></span><strong style={{ color: COLORS.danger }}>− {fmtMoney(totalDistributions)}</strong></div>
            </div>

            {/* Capital table */}
            <div style={S.card}>
              <div style={{ ...S.cardTitle, display: 'flex', alignItems: 'center' }}>📥 Stakeholder Capital<InfoIcon tip="Total money each investor has put into the business. Share % determines their portion of profit distributions." /></div>
              <table style={S.table}>
                <thead><tr><th style={S.th}>Name</th><th style={S.th}>Total Capital</th><th style={S.th}><div style={{ display: 'flex', alignItems: 'center' }}>Share %<InfoIcon tip="This stakeholder's percentage of the total invested capital. Used to divide profit distributions." /></div></th><th style={S.th}>History</th>{isAdmin && <th style={S.th}>Actions</th>}</tr></thead>
                <tbody>
                  {capByName.map((s, i) => {
      ...
      case 'settings': if (!isAdmin) return <Navigate to="/dashboard" replace />; return (
        <div>
          ...
          <div style={S.card}>
            ...
            <div style={S.grid2}>
              <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Daily Interest Rate (%)<InfoIcon tip="Percentage of the cash advance charged per day as a holding fee. Example: 1% on ₦10,000 = ₦100/day. This accumulates every day the customer has not returned." /></span>}>
              ...
              <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Service Fee (₦)<InfoIcon tip="One-time administration charge collected at the start of every new loan, before daily fees begin. This is collected on the same day the cash advance is given." /></span>}>
              ...
              <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Loan Cap No Receipt (%)<InfoIcon tip="Maximum cash advance as a percentage of the item's estimated resale value, when the customer does not have a receipt. Lower cap reduces risk on items with uncertain ownership." /></span>}>
              ...
              <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Loan Cap With Receipt (%)<InfoIcon tip="Maximum cash advance as a percentage of the item's estimated resale value when the customer provides a receipt. Higher cap is allowed because a receipt proves legitimate ownership." /></span>}>
              ...
              <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Max Loan Days<InfoIcon tip="The maximum number of days a customer can take before they must return and repay. The wizard will not allow a longer loan term than this." /></span>}>
              ...
              <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Grace Days<InfoIcon tip="Buffer days added after the internal deadline, before an item becomes eligible for sale. Use this to give customers a final chance to redeem before you list the item." /></span>}>
          ...
          <div style={S.card}>
            ...
            <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>WhatsApp Number<InfoIcon tip="Enter in international format without the + sign (e.g. 2348165491908). Customers can tap to message this number from the public loan check portal." /></span>}>
            ...
            <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Google Maps Link (optional)<InfoIcon tip="Paste a Google Maps share link so customers can get directions. If left blank, a Google Search link is used instead." /></span>}>
          ...
          <div style={S.card}>
            ...
            <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Gemini AI API Key<InfoIcon tip="Your Google Gemini API key, used to power the AI item valuation step during transaction wizards. Get yours free at aistudio.google.com." /></span>}>
            ...
            <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Gemini Model<InfoIcon tip="The specific Gemini AI model to use for valuation. Leave as default unless you have a reason to change it. The system will automatically fall back to other models if the selected one is unavailable." /></span>}>
            ...
            <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>NIN/BVN API Key<InfoIcon tip="Your API key for the identity verification service (checkmyninbvn.com.ng). Used to verify customer NIN and BVN details during the transaction wizard." /></span>}>
          ...
          <div style={S.card}>
            ...
            <Field label={<span style={{ display: 'inline-flex', alignItems: 'center' }}>Require API-verified NIN/BVN to proceed<InfoIcon tip="When ON: staff cannot advance past the Identity step unless the API returned a successful verification with photo. When OFF: any attempt (even a failed one) is enough to proceed — useful when the API is down." /></span>}>

...
export default function App() {
  ...
  const isTxDetailUrl = /^\/transactions\/(?!new$)[^/]+$/.test(location.pathname);
  if (!knownAuthPaths.includes(location.pathname) && !isTxDetailUrl) {
    return <Navigate to="/dashboard" replace />;
  }
  ...
  const renderPage = () => {
    ...
    // Transaction detail page: /transactions/:ref (but not /transactions/new)
    if (isTxDetailUrl) {
      const ref = decodeURIComponent(location.pathname.slice(`${PAGE_PATHS.transactions}/`.length));
      const tx = transactions.find(t => t.ref === ref);
      if (tx) return <TxDetail tx={tx} />;
      if (listLoading) return <div style={{ textAlign: 'center', padding: '40px', color: COLORS.textMuted }}>⏳ Loading transaction...</div>;
      return (<div style={S.card}><p style={{ color: COLORS.textMuted }}>Transaction <strong>{ref}</strong> not found.</p><button style={S.btn('outline')} onClick={() => navigate(PAGE_PATHS.transactions)}>← Back to Transactions</button></div>);
    }

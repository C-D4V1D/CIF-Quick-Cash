...
function ScreeningStep({ tx, upd, onRedFlagExit, onDecline }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '12px', padding: '14px', background: '#fff5f5', borderRadius: '8px', border: `1px solid ${COLORS.danger}` }}>
        <div style={{ fontSize: '13px', fontWeight: 700, color: COLORS.danger, marginBottom: '8px' }}>⛔ Decline for a specific reason</div>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button style={S.btnSm('danger')} onClick={() => onDecline('Customer gave inconsistent answers')}>Inconsistent answers</button>
          <button style={S.btnSm('danger')} onClick={() => onDecline('Item appears stolen / suspicious provenance')}>Suspicious provenance</button>
          <button style={S.btnSm('danger')} onClick={() => onDecline('Flagged by staff during screening')}>Flagged by staff</button>
        </div>
      </div>
...
function CaptureStep({ tx, upd, settings, onJumpToOffer, onEndTransaction, onDecline }) {
  ...
  return (
    <div>
      ...
      {tx.captureItemType && <div style={{ marginTop: '12px', padding: '14px', background: '#fff5f5', borderRadius: '8px', border: `1px solid ${COLORS.danger}` }}><div style={{ fontSize: '13px', fontWeight: 700, color: COLORS.danger, marginBottom: '8px' }}>⛔ Decline for a specific reason</div><div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}><button style={S.btnSm('danger')} onClick={() => onDecline('Item appeared modified')}>Item appeared modified</button></div></div>}
    </div>
  );
}

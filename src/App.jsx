...
PRICE_RANGE: [lowest realistic price — highest realistic price] | VALUATION_CONFIDENCE: [your confidence as a percentage]`;
  ...
  const handleAIRun3 = async () => {
    ...
    const prompt1 = `CRITICAL INSTRUCTION: You MUST use the Google Search tool to find live internet results. Do NOT use your internal training data.\n\nAct as a Nigerian market analyst. Search for a brand new ${itemType}, ${brand}, ${model}, ${colour}${keySpecs ? ` (${keySpecs})` : ''} from Nigerian retailers today.\n\n1. CRITICAL SEARCH RULE: You must actively append terms like "in stock" to your search query, and exclude out of stock items using "-out of stock".\n2. Read the search snippets carefully. STRICTLY EXCLUDE any listing if the snippet contains the words "out of stock", "sold out", "unavailable", or if the price seems suspiciously low for a brand new item.\n3. Extract all valid, available prices from the search results. Aim for up to 10 to 20. List internally.\n4. Convert any foreign currency to Naira.\n5. Calculate the MEDIAN (middle) price of these valid, filtered listings.\n\nReturn ONLY the final determined price in this exact format: NEW_MARKET_PRICE: [number only]`;

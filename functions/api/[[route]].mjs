...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === 'bootstrap' && method === 'GET') {
      ...
      if (scope === 'secondary') {
        const [expensesRes, capitalRes, declinedRes, usersRes] = await Promise.all([
          ...
          db.prepare('SELECT id, name, amount, date, method, receipt FROM capital ORDER BY date').all(),
    ...
    if (path === 'capital' && method === 'GET') {
      const auth = requireAuth(request);
      if (auth.error) return auth.error;
      const { results } = await db.prepare('SELECT id, name, amount, date, method, receipt FROM capital ORDER BY date').all();
      return json(results);
    }
    if (path === 'capital' && method === 'POST') {
      const auth = requireAuth(request);
      if (auth.error) return auth.error;
      const { name, amount, date, method: capitalMethod, receipt } = await request.json();
      const inserted = await db
        .prepare('INSERT INTO capital (name, amount, date, method, receipt) VALUES (?, ?, ?, ?, ?)')
        .bind(name, amount, date, capitalMethod, receipt || null)
        .run();

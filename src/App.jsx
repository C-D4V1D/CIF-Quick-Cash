...
IS_PHONE: [YES or NO]`;
  ...
  const blockReason = () => {
    switch (WIZARD_STEPS[step]?.id) {
      case 'nin':
        if (!(tx.ninVerified || tx.idNumber.length > 5)) return 'You must verify the customer\'s ID before proceeding.';
        break;
      case 'customer':
        if (!tx.fullName) return 'Full name is required.';
        if (!tx.phoneNumbers[0]) return 'Phone 1 is required.';
        if (!tx.familyName || !tx.familyPhone) return 'Family contact name and phone are required.';
        if (!tx.phonesVerified[0] && !tx.phonesVerified[1]) return 'You must mark at least one phone number as called before proceeding.';
        break;
      case 'custPhotos':
        if (!tx.photoCustomerHolding) return 'You must upload a photo of the customer holding the item before proceeding.';
        break;
      case 'itemPhotos':
        if (!(tx.itemPhotos.front || tx.itemPhotos.back)) return 'You must upload at least one item photo (front or back) before proceeding.';
        break;
      case 'aiValuation':
        if (!(tx.aiItemType && tx.estimatedValue > 0)) return 'You must run AI Valuation and confirm the item type and value before proceeding.';
        break;
      case 'imeiSerial':
        if (tx.requiresIMEI) {
          if (!tx.imei) return 'You must enter the IMEI number before proceeding.';
          if (!tx.imeiChecked) return 'You must tick the "Checked on imei.info" checkbox before proceeding.';
          if (tx.imeiClean === null) return 'You must select Clean or Flagged after checking the IMEI before proceeding.';
        }
        break;
      case 'offer':
        if (!tx.cashAdvance) return 'You must enter the cash advance amount before proceeding.';
        if (!tx.dateGiven) return 'You must set the date given before proceeding.';
        break;
      case 'agreement':
        if (!tx.photoSigning) return 'You must upload a photo of the signed agreement before proceeding.';
        break;
      default: return '';
    }
    return '';
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '12px' }}>
        {step < WIZARD_STEPS.length - 1 && !canProceed() && blockReason() && (
          <div style={{ ...S.alert('danger'), marginBottom: '8px', textAlign: 'right' }}>⛔ {blockReason()}</div>
        )}
